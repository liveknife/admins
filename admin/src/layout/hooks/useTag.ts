...
export function useTags() {
  ...
  watchEffect(() => {
    tagTextKeys.forEach((key, index) => {
      if (index === 6 && pureSetting.hiddenSideBar) {
        tagsViews[index].text = t("tags.contentExitFullscreen");
        return;
      }
      tagsViews[index].text = t(key);
    });
  });
